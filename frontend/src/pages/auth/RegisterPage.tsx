...
import { User, Mail, Lock, AlertCircle, Loader2, ArrowLeft } from "lucide-react";
...
function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirect ke dashboard atau todos
      navigate("/todos");
  ...
  return (
    <div className="animate-in fade-in slide-in-from-right-4 duration-700">
      ...
      <form onSubmit={handleRegister} className="space-y-5">
        ...
        <div className="space-y-2">
          ...
          <div className="relative group">
            <User className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-[#2557bd] transition-colors" size={18} strokeWidth={1.5} />
            <input
              type="text"
      ...
      <div className="mt-12 text-center text-sm font-light text-slate-400">
        ...
        <Link 
          to="/login" 
          className="text-[#2557bd] font-normal hover:underline ml-1 transition-colors"
        >
          Sign In
        </Link>
      </div>
